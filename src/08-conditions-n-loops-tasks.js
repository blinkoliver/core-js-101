...
const findFirstSingleChar = (str) => {
  for (let index = 0; index < str.length; index += 1) {
    if (str.indexOf(str[index]) === str.lastIndexOf(str[index])) return str[index];
  }
  return null;
};
/**
...
const getIntervalString = (a, b, isStartIncluded, isEndIncluded) => {
  let firstBracket; let
    secondBracket;
  if (isStartIncluded) { firstBracket = '['; } else { firstBracket = '('; }
  if (isEndIncluded) { secondBracket = ']'; } else { secondBracket = ')'; }
  if (a < b) {
    return `${firstBracket}${a}, ${b}${secondBracket}`;
  }
  return `${firstBracket}${b}, ${a}${secondBracket}`;
};
/**
 ...
 * 'noon' => 'noon'
 */
const reverseString = (str) => (str.includes(' ')
  ? str
    .split(' ')
