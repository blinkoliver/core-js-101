 ...
 *   'cat'       => 'c'
 */
const getFirstChar = (value) => value.charAt(0);
/**
 ...
 */
const removeLeadingAndTrailingWhitespaces = (value) => value.replace(/\s\s+/g, ' ').trim();
/**
...
const repeatString = (value, count) => {
  let result = '';
  while (result.length < count * value.length) {
    result += value;
  }
  return result;
};
...
const removeFirstOccurrences = (str, value) => {
  ...
  const secondPart = str.slice(str.indexOf(value) + value.length, str.length);
 ...
 *   '<a>' => 'a'
 */
const unbracketTag = (str) => str.trim().slice(1, -1);
/**
...
const getRectangleString = (width, height) => {
  ...
  const leftBottom = '└';
  const rightBottom = '┘';
  const header = `${lefTop + horizontal.repeat(width - 2) + rightTop}\n`;
  const main = `${vertical + ' '.repeat(width - 2) + vertical}\n`.repeat(
    height - 2,
  );
  const footer = `${leftBottom + horizontal.repeat(width - 2) + rightBottom}\n`;
